...
The `stringr` package matches **patterns** in strings. But how can we specify generic patterns ?
...
# get numbers only:
str_extract(my_string, "\\d+")  # \\d = digits; + = at least 1 time
str_extract(my_string, "[0-9]+")
```
...
# extract all letters:
str_extract_all(my_string, "[A-z]")  # upper and lower case letters
str_extract(my_string, "[A-z]+") # stops when encounters a different charater (" ")
str_extract(my_string, "[A-z|\\s]+") # add that white spaces ("\\s")are to be extracted too
```
...
# are there any double "l" ?
str_detect(my_string, "l{2}")

# remove trailing whitespace and dot:
str_replace(my_string, "\\s\\.$", "")

# keep only the email:
str_replace(my_string, "^[^c]*", "")
```

## Working on strings

![](images/regex.png){width=100%}
...
Extract different elements of a date time:

```{r}
oneday <- "2029-10-24 00:02:00"
date(oneday)
minute(oneday)
second(oneday)
year(oneday)
...
#### Different ways to deal with time!
